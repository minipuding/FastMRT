...
from fastmrt.pretrain.transforms import FastmrtPretrainTransform, FastmriPreprocessingTransform
...
def build_args():
    ...
    parser.add_argument('--ckpt', type=str, default="",
                        help="ckpt directory of pretrained model")
...
class FastmrtRunner:
    ...
    def __init__(self, args, log_cfgs, source_cfgs) -> None:
        ...
        if args.net == "pretrain":
            self.train_transform = transforms.Compose([FastmriPreprocessingTransform(), FastmrtPretrainTransform(use_random_seed=True), train_transform])
            self.val_transform = transforms.Compose([FastmriPreprocessingTransform(), FastmrtPretrainTransform(use_random_seed=False), val_transform])
        else:
    ...
    def _get_model(self, args):
        ...
        if args.net == 'zf':
        ...
        elif args.net == 'resunet' or args.net == 'pretrain':
            model = ResUnet(
                in_channels=args.model_in_channels,
                out_channels=args.model_out_channels,
                base_channels=args.model_base_channels,
            ...
            if args.ckpt:        
                model_state_dict = torch.load(args.ckpt)["state_dict"]
                temp_state_dict = {}
